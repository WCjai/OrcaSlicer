...
# Unbound3D-slicer Flatpak Build Script
# This script builds and packages Unbound3D-slicer as a Flatpak package locally
...
show_help() {
    ...
    echo ""
    echo "Build Unbound3D-slicer as a Flatpak package"
    echo ""
    echo "Options:"
...
echo -e "${BLUE}Unbound3D-slicer Flatpak Build Script${NC}"
echo -e "${BLUE}================================${NC}"
...
if [[ ! -f "./scripts/flatpak/com.unbound3d.Unbound3DSlicer.yml" ]]; then
    echo -e "${RED}Error: Flatpak manifest not found at scripts/flatpak/com.unbound3d.Unbound3DSlicer.yml${NC}"
    exit 1
fi
...
BUNDLE_NAME="Unbound3D-slicer-Linux-flatpak_${VER}_${ARCH}.flatpak"
...
MANIFEST="scripts/flatpak/com.unbound3d.Unbound3DSlicer.yml"
if [[ "$NO_DEBUGINFO" == true ]]; then
    MANIFEST="scripts/flatpak/com.unbound3d.Unbound3DSlicer.no-debug.yml"
    sed '/^build-options:/a\  no-debuginfo: true\n  strip: true' \
        scripts/flatpak/com.unbound3d.Unbound3DSlicer.yml > "$MANIFEST"
...
if ! flatpak-builder \
    ...
    rm -f "scripts/flatpak/com.unbound3d.Unbound3DSlicer.no-debug.yml"
    exit 1
fi
