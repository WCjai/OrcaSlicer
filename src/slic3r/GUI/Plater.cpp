...
void Sidebar::priv::flush_printer_sync(bool restart)
{
    ...
    //btn_sync_printer->SetBackgroundColorNormal((*counter_sync_printer & 1) ? "#F8F8F8" :"#333333");
...
{
    StateColor not_synced_colour(std::pair<wxColour, int>(wxColour("#333333"), StateColor::Normal));
    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(39 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    // add printer
    {
        ...
        struct PanelColors {
            ...
            wxColour bg_focus  = "#F0F0F0";
            wxColour bd_normal = "#DBDBDB";
            wxColour bd_hover  = "#333333";
            wxColour bd_focus  = "#333333";
        };
        ...
        */
        ...
        StateColor btn_sync_bd_col(
                std::pair<wxColour, int>(wxColour("#333333"), StateColor::Pressed),
                std::pair<wxColour, int>(wxColour("#333333"), StateColor::Hovered),
    ...
    text_ctrl->Bind(wxEVT_SET_FOCUS, [this](wxFocusEvent& e) {
        ...
        p->m_search_bar->SetBorderColor(wxColour("#333333"));
...
void Sidebar::sync_ams_list(bool is_from_big_sync_btn)
{
    ...
    if (!unknowns.empty()) {
        MessageDialog dlg(this,
            _L("There are some unknown or incompatible filaments mapped to generic preset.\nPlease update Unbound3D Slicer or restart Unbound3D Slicer to check if there is an update to system presets.") + detail,
            _L("Sync filaments with AMS"), wxOK);
        dlg.ShowModal();
    }
    if (!sync_color_only) {
                ...
                En3mfType          en_3mf_file_type = En3mfType::From_BBS;
                {
                    ...
                    if (en_3mf_file_type == En3mfType::From_Prusa) {
                        ...
                        if(load_type != LoadType::LoadGeometry)
                            show_info(q, _L("The 3MF is not supported by Unbound3D Slicer, loading geometry data only."), _L("Load 3MF"));
                    }
                    ...
                    // Orca: check if the project is created with OrcaSlicer 2.3.1-alpha and use the sparse infill rotation template for non-safe infill patterns
                        ...
                        if (!config_loaded.opt_string("sparse_infill_rotate_template").empty()) {
                            ...
                            if (!is_safe_to_rotate) {
                                ...
                                msg_text += "\n\n" +
                                            _(L("Would you like Unbound3D Slicer to automatically fix this by clearing the rotation template settings?"));
                        ...
                        load_config = false;
                        show_info(q, _L("The 3MF file was generated by an old version, loading geometry data only."), _L("Load 3MF"));
                    }
                    else if (!load_config) {
...
void Plater::priv::apply_color_mode()
{
    ...
    wxColour   orca_color      = wxColour(42, 42, 42);//wxColour(ColorRGBA::ORCA().r_uchar(), ColorRGBA::ORCA().g_uchar(), ColorRGBA::ORCA().b_uchar());
    orca_color                 = is_dark ? StateColor::darkModeColorFor(orca_color) : StateColor::lightModeColorFor(orca_color);
    wxColour sash_color = is_dark ? wxColour(42, 42, 42) : wxColour(206, 206, 206);
...
#else
    wxGetApp().mainframe->SetTitle(m_project_name + " - Unbound3D Slicer");
...
void Plater::priv::bring_instance_forward() const
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "Unbound3D Slicer window going forward";
            ...
            http.on_progress([&percent, &cont, &msg, &filesize, &size_limit](Http::Progress progress, bool& cancel) {
                ...
                .on_error([&msg, &cont, &retry_count, max_retries](std::string body, std::string error, unsigned http_status) {
                    ...
                    if (retry_count == max_retries) {
                        msg = _L("Importing to Unbound3D Slicer failed. Please download the file and manually import it.");
                        cont = false;
                    }
                })
    ...
    , m_action(2)
{
    ...
    m_fname_f->Wrap(-1);
    m_fname_f->SetForegroundColour(wxColour(42, 42, 42));
    ...
    m_fname_s->Wrap(-1);
    m_fname_s->SetForegroundColour(wxColour(42, 42, 42));
    ...
    if (type == PrinterWarningType::NOT_CONNECTED) {
        if (device_page) {
            ...
            content = wxString::Format(
                _L("Unbound3D Slicer can't connect to %s. Please check if the printer is powered on and connected to the network."), printer_name);
        }
...
void Plater::show_object_info()
{
    ...
    if (non_manifold_edges > 0) {
        info_manifold += into_u8("\n" + _L("Tips:") + "\n" +_L("\"Fix Model\" feature is currently only on Windows. Please repair the model on Unbound3D Slicer (Windows) or CAD softwares."));
    }
    #endif //APPLE & LINUX
