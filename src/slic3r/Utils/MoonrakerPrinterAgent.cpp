    ...
    while (!ws_stop.load() && retry_count < max_retries) {
        ...
        try {
            ...
            ws.set_option(websocket::stream_base::decorator([&](websocket::request_type& req) {
                req.set(http::field::user_agent, "Unbound3D-slicer");
                if (!api_key.empty()) {
            ...
            identify["params"]["client_name"] = "Unbound3D-slicer";
            identify["params"]["version"]     = MoonrakerPrinterAgent_VERSION;
            identify["params"]["type"]        = "agent";
            identify["params"]["url"]         = "https://github.com/Unbound3D/Unbound3D-slicer";
            identify["id"]                    = 0;
