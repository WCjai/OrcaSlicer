...
public:
    ...
    bool load_opengl_dll()
    {
        ...
        wc.lpszClassName = L"Unbound3D_opengl_version_check";
        wc.style = CS_OWNDC;
        if (RegisterClass(&wc)) {
            HWND hwnd = CreateWindowW(wc.lpszClassName, L"Unbound3D_opengl_version_check", WS_OVERLAPPEDWINDOW, 0, 0, 640, 480, 0, 0, wc.hInstance, (LPVOID)this);
            if (hwnd) {
...
extern "C" {
    ...
    Slic3rMainFunc unbound3d_main = nullptr;
}
...
// http://download.qt.io/development_releases/prebuilt/llvmpipe/windows/
    ...
    wcscat(path_to_slic3r, L"Unbound3D-slicer.dll");
//	printf("Loading Slic3r library: %S\n", path_to_slic3r);
    HINSTANCE hInstance_Slic3r = LoadLibraryExW(path_to_slic3r, nullptr, 0);
    if (hInstance_Slic3r == nullptr) {
        printf("Unbound3D-slicer.dll was not loaded, error=%d\n", GetLastError());
        return -1;
    }

    // resolve function address here
    unbound3d_main = (Slic3rMainFunc)GetProcAddress(hInstance_Slic3r,
#ifdef _WIN64
    ...
    if (unbound3d_main == nullptr) {
        printf("could not locate the function unbound3d_main in Unbound3D-slicer.dll\n");
        return -1;
    }
    // argc minus the trailing nullptr of the argv
    return unbound3d_main((int)argv_extended.size() - 1, argv_extended.data());
}
}
